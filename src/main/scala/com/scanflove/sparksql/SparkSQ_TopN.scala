...
object SparkSQ_TopN {
    def main(args: Array[String]): Unit = {
        ...
        val spark: SparkSession = SparkSession.builder()
                ...
                .config("spark.yarn.jars","hdfs://lv/spark/jars/*.jar")//把lib目录所有依赖传到hdfs，不需要吧依赖打包到目的jar
...
class CityClickUDAF extends UserDefinedAggregateFunction{
    ...
    override def evaluate(buffer: Row): Any = {
        ...
        if(cityCountMap.size > 2){
            citysRatio = citysRatio :+ CityRemark("其它",citysRatio.foldRight(1D)(_.cityRatio-_))
        }
